{{-- resources/views/domains/waterdamage/blog/partials/featured-posts.blade.php --}}
            ...
            @if($featuredPosts->first())
                ...
                <article class="group relative bg-white/10 backdrop-blur-sm rounded-xl border border-white/20 overflow-hidden hover:bg-white/15 transition-colors h-full">
                    <a href="{{ route('blog.show', $post->slug) }}" class="block h-full">
                        ...
                        @else
                            <div class="p-6 md:p-8 flex flex-col justify-end h-full">
                                <h3 class="text-xl md:text-2xl font-bold text-white mb-3 line-clamp-2">
                                    <a href="{{ route('blog.show', $post->slug) }}" class="hover:text-emerald-400 transition">
                                        {{ $post->title }}
                                    </a>
                                </h3>
                                <p class="text-slate-300 text-sm line-clamp-2 mb-4">{!! Str::limit(strip_tags($post->excerpt), 120) !!}</p>
                ...
                <article class="group bg-white/10 backdrop-blur-sm rounded-xl border border-white/20 overflow-hidden hover:bg-white/15 transition-colors">
                    <a href="{{ route('blog.show', $post->slug) }}" class="flex gap-4 p-4">
                        <div class="flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden bg-gradient-to-br from-emerald-100 to-emerald-200">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center text-2xl opacity-50">&#x1FABD;</div>
                            @endif
                        </div>
                        <div class="flex-1 min-w-0">
                            <h3 class="text-sm font-bold text-white mb-1 line-clamp-2 group-hover:text-emerald-400 transition-colors">
                                {{ $post->title }}
                            </h3>
