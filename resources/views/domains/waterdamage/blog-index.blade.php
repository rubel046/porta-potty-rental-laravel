...
<div class="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white py-3">
    <div class="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center justify-center gap-2 text-center md:text-left text-xs sm:text-sm">
        <div class="flex items-center gap-2">
            <span>&#x2B50;</span>
            <span class="font-semibold">4.9/5 Rating (500+ Reviews)</span>
        </div>
        <span class="hidden md:inline text-emerald-100">|</span>
...
<section class="relative py-16 md:py-20 overflow-hidden">
    ...
    <div class="relative max-w-4xl mx-auto px-4 text-center">
        <h1 class="text-4xl md:text-5xl font-extrabold text-white mb-5">Water Damage Restoration Blog</h1>
        <p class="text-xl text-slate-300">Expert guides, tips, and resources</p>
    </div>
</section>
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="md:hidden mb-6">
                    <div class="flex gap-2 overflow-x-auto pb-2">
                        <a href="{{ route('blog.index') }}"
                           class="flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium {{ !$selectedCategory ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-700' }}">
                            All ({{ $totalPostsCount ?? $posts->total() }})
                        </a>
                        @foreach($categories as $cat)
                            <a href="{{ route('blog.category', ['slug' => $cat->slug]) }}"
                               class="flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium {{ $selectedCategory?->id === $cat->id ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-700' }}">
                ...
                <div class="mb-6 flex items-center justify-between">
                    <p class="text-sm text-slate-500">
                        @if($selectedCategory) <span class="font-semibold text-slate-700">{{ $selectedCategory->name }}</span>
                    ...
                    <div class="flex gap-1">
                        <a href="{{ route('blog.index', array_merge(request()->query(), ['sort' => 'latest'])) }}"
                           class="px-3 py-1 text-xs font-medium rounded-full {{ !request('sort') || request('sort') === 'latest' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600' }}">Latest</a>
                        <a href="{{ route('blog.index', array_merge(request()->query(), ['sort' => 'popular'])) }}"
                           class="px-3 py-1 text-xs font-medium rounded-full {{ request('sort') === 'popular' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600' }}">Popular</a>
                    </div>
                </div>
                    ...
                    @forelse($posts as $post)
                        <article class="group bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all">
                            <a href="{{ route('blog.show', $post->slug) }}" class="block">
                                ...
                                @else
                                    <div class="aspect-[16/9] bg-gradient-to-br from-blue-100 to-emerald-50 flex items-center justify-center">
...
<section class="py-12 px-4 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-center">
    <div class="max-w-3xl mx-auto">
        <h2 class="text-2xl md:text-3xl font-bold mb-3">Need Water Damage Help?</h2>
        <a href="tel:{{ domain_phone_raw() }}" class="inline-flex items-center gap-2 bg-white text-emerald-600 font-bold text-xl py-3 px-8 rounded-full hover:scale-105 transition-all">
