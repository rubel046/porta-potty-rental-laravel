...
style="display: none;">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-scale-in"
        ...
        <div class="p-8 text-center">
            {{-- Icon --}}
            <div class="w-20 h-20 bg-gradient-to-br from-amber-400 to-amber-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-amber-500/30">
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
                <ul class="text-sm text-amber-800 space-y-2 text-left">
                    <li class="flex items-center gap-2"><span class="text-amber-600">✓</span> Same-day delivery available</li>
                    <li class="flex items-center gap-2"><span class="text-amber-600">✓</span> Free delivery & pickup</li>
                    <li class="flex items-center gap-2"><span class="text-amber-600">✓</span> No hidden fees, transparent pricing</li>
                </ul>
            </div>
            
            {{-- Phone CTA --}}
            <a href="{{ domain_phone_link() }}" class="block w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-700 text-white text-xl font-bold py-4 rounded-xl shadow-lg shadow-amber-500/30 transition-all hover:scale-[1.02] mb-4">
                📞 {{ domain_phone_display() }}
            </a>
            
            {{-- Dismiss link --}}
            <button @click="dismissed = true" class="text-slate-400 hover:text-slate-600 text-sm transition">No thanks</button>
        </div>
