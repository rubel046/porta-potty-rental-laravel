...
@section('page-title', 'Add Phone Number')
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="number" value="{{ old('number') }}" required 
                    ...
                    class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-mono focus:ring-2 focus:ring-green-500 focus:border-green-500">
                <p class="text-xs text-gray-400 mt-1">Area code will be extracted automatically</p>
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Friendly Name</label>
                <input type="text" name="friendly_name" value="{{ old('friendly_name') }}" 
                    placeholder="(713) 555-1234 - Houston Main" 
            ...
            <div x-data="{ open: false, search: '', selectedId: '', selectedName: 'No city assigned' }">
                ...
                <div class="relative">
                    ...
                    <div x-show="open" x-transition.opacity style="display: none;" class="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg">
                        <div class="p-2 border-b border-gray-100">
                            <input type="text" x-model="search" placeholder="Search cities..." 
                        ...
                        <div class="overflow-y-auto max-h-64">
                            <button type="button" @click="search = ''; selectedId = ''; selectedName = 'No city assigned'; open = false" 
                                class="w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50"
                                :class="search === '' ? 'bg-green-50 text-green-700 font-medium' : ''">
                                No city assigned
                            </button>
                            @foreach($cities as $city)
                                <button type="button" 
                                    ...
                                    class="w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50"
                                    x-show="'{{ strtolower($city->name) }}'.includes(search.toLowerCase()) || search === ''">
