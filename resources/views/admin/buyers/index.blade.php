...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h2 class="text-lg font-semibold text-gray-800">All Buyers</h2>
        <p class="text-sm text-gray-500">Manage your lead buyers and call routing</p>
    </div>
    <a href="{{ route('admin.buyers.create') }}" class="inline-flex items-center gap-2 bg-green-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-green-700 transition shadow-sm">
    ...
    <form method="GET" class="flex flex-wrap gap-4 items-end">
        ...
        <div class="flex gap-2">
            ...
            @if(request()->anyFilled(['search', 'status']))
                <a href="{{ route('admin.buyers.index') }}" class="px-4 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50 transition">
                    Clear
                </a>
            @endif
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50/50 text-left text-xs text-gray-500 uppercase tracking-wider border-b border-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-4 font-medium">Payout/Call</th>
                    <th class="px-6 py-4 font-medium">Status</th>
                    <th class="px-6 py-4 font-medium">Calls Today</th>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($buyers as $buyer)
                    <tr class="hover:bg-gray-50/50 transition">
                        <td class="px-6 py-4">
                            <div class="font-medium text-gray-900">{{ $buyer->company_name }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-gray-900">{{ $buyer->contact_name }}</div>
                            <div class="text-xs text-gray-400">{{ $buyer->phone }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-gray-900 font-medium">${{ number_format($buyer->payout_per_call, 2) }}</span>
                        </td>
                        <td class="px-6 py-4">
                            @if($buyer->is_active)
                        ...
                        <td class="px-6 py-4">
                            <span class="@if($buyer->today_calls > 0) text-green-600 font-medium @else text-gray-400 @endif">
                                {{ $buyer->today_calls }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.buyers.show', $buyer) }}" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition" title="View">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                <a href="{{ route('admin.buyers.edit', $buyer) }}" class="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <form method="POST" action="{{ route('admin.buyers.destroy', $buyer) }}" onsubmit="return confirm('Are you sure you want to delete this buyer?')" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500 font-medium">No buyers found</p>
    ...
    @if($buyers->hasPages())
        <div class="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div class="text-sm text-gray-500">
                {{ $buyers->firstItem() }} - {{ $buyers->lastItem() }} of {{ $buyers->total() }}
            </div>
            <nav class="flex items-center gap-1">
                @if($buyers->currentPage() > 1)
                    <a href="{{ $buyers->previousPageUrl() }}" class="px-3 py-1.5 text-sm rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition">Previous</a>
                @endif
                
                @foreach($buyers->getUrlRange(max(1, $buyers->currentPage() - 2), min($buyers->lastPage(), $buyers->currentPage() + 2)) as $page => $url)
                ...
                @if($buyers->currentPage() < $buyers->lastPage())
                    <a href="{{ $buyers->nextPageUrl() }}" class="px-3 py-1.5 text-sm rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition">Next</a>
                @endif
            </nav>
        </div>
    @endif
