...
<div class="mb-4 flex flex-wrap justify-between items-center gap-4">
    <form method="GET" class="flex gap-2 flex-wrap">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Invoice #..." class="border border-gray-200 rounded-lg px-3 py-2 text-sm">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($invoices as $invoice)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-3 font-mono text-xs font-medium">{{ $invoice->invoice_number }}</td>
                        ...
                        <td class="px-6 py-3 font-bold text-green-600">${{ number_format($invoice->total_amount, 2) }}</td>
                        <td class="px-6 py-3">
    ...
    <div class="px-6 py-4 border-t border-gray-100 flex justify-between items-center">
        {{ $invoices->links() }}
        <div class="text-sm text-gray-500">{{ $invoices->total() }} invoices</div>
    </div>
</div>
@endsection
